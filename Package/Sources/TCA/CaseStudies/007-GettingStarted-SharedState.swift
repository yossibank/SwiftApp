...
複数の独立した画面がComposable Architectureでどのように状態を共有するかを示しています。

各タブは独自の状態を管理し、別々のモジュールにある可能性がありますが、一つのタブでの変更は他のタブに即座に反映されます。

このタブには独自の状態があり、カウント値の増減や、現在のカウントが素数であるかどうかを尋ねる際に設定されるアラート値が含まれています。

内部的には、最小および最大カウント数や、発生したカウントイベントの総数など、さまざまな統計も追跡しています。

これらの状態は他のタブで表示でき、統計は他のタブからリセットできます。
"""
