...
バインディング状態とアクションを使用することで、各UIコントロールにユニークなアクションが必要となることによる定型文を安全に排除することができます。

代わりに、すべてのUIバインディングは単一のbindingアクションに統合されることができ、そのアクションはBindingAction値を保持し、すべてのバインディング状態はBindingStateプロパティラッパーで保護されることができます。
