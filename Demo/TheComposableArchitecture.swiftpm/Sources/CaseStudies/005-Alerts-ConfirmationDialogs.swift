...
ライブラリはアプリケーション内のすべてのデータフローが単一方向であることを要求しているため、状態をリデューサーを介さずに変更できるSwiftUIの双方向バインディングを利用することはできません。これは、標準APIを直接使用してアラートやシートを表示することができないことを意味します。

しかし、ライブラリにはAlertStateとConfirmationDialogStateの2つのタイプがあり、これらはリデューサーから構築することができ、アラートや確認ダイアログが表示されるかどうかを制御できます。さらに、ボタンをタップしたときにアクションを自動的に送信する機能も備えており、双方向バインディングやアクションクロージャーではなく、リデューサー内でその機能を適切に扱うことができます。
